    ...
    void Update(SDL_Renderer* renderer) {
        for(auto& entity : GetEntities()) {
            ...
            if(debug.isRenderBoxCollider) {
                RenderBoxCollider(renderer, entity);
            }
        }
    }

    void RenderBoxCollider(SDL_Renderer* renderer, const Entity& entity) {
        ...
        SDL_Rect rt;
        rt.x = static_cast<int>(transform.position.x + boxCollider.offset.x);
        rt.y = static_cast<int>(transform.position.y + boxCollider.offset.y);
